...
class ImageService:
    ...
    def download_file(self, url: str, temp_path: str) -> bool:
    ...
    @staticmethod
    def process_all_products(store_type: str = None):
        """Process images and videos for all products needing upload"""
        try:
            ...
            with DatabaseManager.get_connection() as conn:
                ...
                # Build query based on store_type
                base_query = """
                    SELECT p.id, p.store_id, p.image_url, p.source_image_url, p.product_images, p.video_url, s.store_type
                    FROM products p
                ...
                logger.info(f"Processing media for {len(products)} products")
                ...
                def process_single_product(product):
                    ...
                    try:
                        ...
                        if not image_service.download_file(source_url, str(temp_path)):
                            logger.error(
                                f"Failed to download image for product ID {product_id}: {source_url}"
                            )
                            return (product_id, None, None, None)

                        # Determine R2 folder based on store type
                        folder = (
                            "woo" if current_store_type == "woocommerce" else "starter"
                        )
                        original_filename = source_url.split("/")[-1]
                        r2_key = f"{folder}/images/{original_filename}"
                        ...
                        # Process video (WooCommerce only)
                        r2_video_url = None
                        if video_url and not video_url.startswith(
                            ...
                            if image_service.download_file(
                                ...
                                if image_service.compress_video(
                                    ...
                                    video_r2_key = (
                                        f"{folder}/videos/{video_url.split('/')[-1]}"
                                    )
                        ...
                        return (product_id, r2_url, processed_images, r2_video_url)
                ...
                with ThreadPoolExecutor(max_workers=20) as executor:
                    ...
                    for future in as_completed(futures):
                        product_id, r2_url, product_images, video_r2_url = (
                            future.result()
                        )
                            ...
                            cursor.execute(
                                """UPDATE products
                                   SET image_url = %s, product_images = %s,
                                       video_url = COALESCE(%s, video_url), updated_at = updated_at
                                   WHERE id = %s""",
                                (r2_url, product_images, video_r2_url, product_id),
                            )
                            conn.commit()
                            success += 1
                ...
                logger.info(
                    f"Media processing complete: {success} success, {failed} failed"
                )
